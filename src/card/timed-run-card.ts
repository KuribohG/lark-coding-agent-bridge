...
export function timedRunForm(scope: string, agentKind: AgentKind, environment: ModelEnvironment, timeZone: string): object {
  return {
    schema: '2.0', config: { summary: { content: '一次性限时任务' } },
    header: { title: { tag: 'plain_text', content: '⏱ 一次性限时任务' }, template: 'blue' },
    body: { elements: [
      { tag: 'markdown', content: '使用当前话题/聊天的上下文。模型和强度只覆盖这次任务；提交后先核对具体停止时间，再点击启动。\n到期、出错或额度耗尽后不自动续跑。服务端已接收请求的计费以服务商规则为准。' },
      { tag: 'form', name: 'timed_run', elements: [
        { tag: 'markdown', content: '**任务目标 / 任务列表**' },
        { tag: 'input', name: 'task', input_type: 'multiline_text', required: true,
          placeholder: { tag: 'plain_text', content: '请描述要完成的工作；完成后提前结束。' } },
        { tag: 'markdown', content: '**截止时间**：01:00 表示下一次凌晨一点；也可填写 2h、90m 或带时区的完整日期。' },
        { tag: 'input', name: 'until', default_value: '2h', required: true },
        { tag: 'markdown', content: '**时区**（01:00 等钟点按此时区解释）' },
        { tag: 'input', name: 'time_zone', default_value: timeZone, required: true },
        { tag: 'markdown', content: '**提前停止的安全余量（分钟）**：默认 5；收尾也计入时间窗口。' },
        { tag: 'input', name: 'margin', default_value: '5' },
        { tag: 'markdown', content: '**本次模型**' },
        { tag: 'select_static', name: 'model_pick', initial_option: 'default', options: [
          { text: { tag: 'plain_text', content: '沿用当前对话设置' }, value: 'default' },
          { text: { tag: 'plain_text', content: '使用下方手填模型名' }, value: '__manual__' },
          ...environment.models.filter((m) => m.value !== 'default').slice(0, 80).map((m) => ({
            text: { tag: 'plain_text', content: m.label }, value: m.value,
          })),
        ] },
        { tag: 'input', name: 'model', placeholder: { tag: 'plain_text', content: 'provider/model' } },
        { tag: 'markdown', content: '**本次思考强度**' },
        { tag: 'select_static', name: 'effort', initial_option: 'default', options:
        ...
        { tag: 'button', name: 'preview', type: 'primary', form_action_type: 'submit',
          text: { tag: 'plain_text', content: '核对任务与停止时间' },
          behaviors: [{ type: 'callback', value: { cmd: 'run.submit', settings_scope: scope } }],
        },
      ] },
    ] },
  };
