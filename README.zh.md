...
|---|---|
| `/new`, `/reset` | 清空当前会话 |
| `/btw <问题>` | 在当前话题/聊天回答旁问，并记住之前的旁问问答 |
| `/cd <path>` | 切换工作目录并重置会话 |
| `/ws list` | 列出命名工作空间 |
...
| `/agent [codex\|claude]` | 全局切换此 bot 的执行引擎（owner/admin），重启后保留 |
...
`/btw 问题` 在当前话题内（普通群或私聊则在当前聊天）返回一个标有“旁问 /btw”的答复。仅提供 `/btw`，没有别名。Claude 和 Codex 使用相同的交互方式；需要当前话题/聊天已建立主会话。

收到问题后会添加“正在输入”表情反馈，排队和执行期间保留，完成或取消后移除。如果问题在答复发出前被撤回，会尝试回复原话题根消息（需要有效的根消息 ID）；非话题消息则直接发到原聊天。

每次旁问从主会话当前已保存的上下文创建独立分叉，并带上此前成功的旁问问答。旁问按话题独立排队，不合并，不切换主会话；主任务继续运行，有可用进程槽位时可同时回答。旁问被要求仅根据已有上下文回答，不继续主任务。

记录按 bot profile 和话题/聊天隔离，同一话题内的用户共用；bridge 重启后保留。最多回放最近 20 组问答，总计约 60000 字符；过长的单条历史会截断。`/new`、切换工作区和成功 `/resume` 会清空该范围的旁问历史，`/stop` 同时取消主任务及运行中/排队中的旁问。旁问问答不会通过自动话题历史导入主会话，显式引用旁问答复仍可将它带入主会话。

## 一次性限时任务

发送 `/run` 打开表单，填写任务、截止时间、时区、安全余量和本次模型/思考强度，模型名支持手填。提交后先显示预览，**创建者核对具体日期和强制停止时间、点击确认后才会运行**。也可用命令生成预览：
...
`/run defaults` 打开默认值表单，`/run defaults reset` 恢复内置的两小时时限、五分钟余量、本机时区及沿用对话的模型/强度。默认值属于当前 bot profile，单独存于 `profiles/<profile>/run-defaults.<agent>.json`，不影响普通聊天设置或其他 bot。任务表单会自动填入默认值；命令显式参数只覆盖本次任务，`--model default`、`--effort default` 表示本次沿用对话设置。默认截止时间只保存钟点或时长，不保存固定日期；每次生成新预览重新计算绝对截止时间，已有预览和运行中的任务不变。若下一次钟点已落入安全余量内，本次拒绝启动，不会顺延一天。简写仍需确认，也不会每天自动执行。
...
任务使用当前话题/聊天的上下文，本次模型/强度保存为任务快照，不修改 `/model`、`/effort` 或 bot 默认值；其他话题/聊天不受影响。应提供具体目标或任务列表，完成后提前结束，不会为了耗尽额度无限循环。

固定停止时间覆盖进程槽位排队和受监管的 CLI 进程树，过期任务不能启动。可识别的明确额度耗尽错误会终止任务，普通限流不会误判为额度耗尽。可用 `/run stop`、`/stop` 或任务卡停止，`/run status` 查看最新/正在运行的任务。仅创建者可启动；创建者或管理员可用任务卡停止，普通 `/stop` 权限保持原样。当前话题已有任务时需先等它结束或停止；限时任务期间收到的普通消息、agent 卡片回调会明确提示未排队，避免截止后触发无限时续跑。

任务持久化到 `profiles/<profile>/timed-runs.json`。已启动任务不能重复启动；停止、到期、失败或 bridge 重启后都不会自动继续，续做需新建任务。第一版没有每日自动调度或服务商余额查询。Linux 进程树清理已由进程测试覆盖，macOS/Windows 实现尚未实机验证。本地终止不能保证服务商对已接收请求的跨额度周期计费方式，严格的费用限制仍需服务端支持。
...
owner/admin 可以发送 `/agent` 打开切换卡片，或直接发送 `/agent codex`、`/agent claude`；`/config` 也有入口。选择保存到当前 bot profile 的 `agentKind`，对其所有私聊、群聊和话题的后续任务生效，重启后保留。切换不需要断开飞书连接，其他 bot profile 不受影响。

两种引擎分别保留可恢复会话、模型/思考强度和限时任务默认值；首次切到另一引擎时使用其 CLI 默认设置。CLI 必须已安装并配置好认证。存在运行中、排队中的任务、旁问或会议时拒绝切换；预检或保存失败时继续使用原引擎。切换前打开的模型/配置表单需要重新打开，限时任务预览只能用创建时的引擎启动。已有历史会话仍受原工作目录和权限匹配规则约束。
